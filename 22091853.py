...
sales_by_year.plot(marker='o', linestyle='-', color='purple',
                   linewidth=2, markersize=8, label='Total Sales')

# Add a trendline using NumPy's polyfit
z = np.polyfit(sales_by_year.index, sales_by_year.values, 1)
p = np.poly1d(z)
plt.plot(sales_by_year.index, p(sales_by_year.index),
         linestyle='--', color='gray', label='Trendline')

max_sales_year = sales_by_year.idxmax()
max_sales_value = sales_by_year.max()
plt.scatter(max_sales_year, max_sales_value, color='red',
            label=f'Highest Sales\nYear: {max_sales_year}\nSales: {max_sales_value:.2f}')
...
plt.subplot(2, 2, 2)
average_sales_by_product_line = df.groupby(
    'PRODUCTLINE')['SALES'].mean().sort_values(ascending=False)
average_sales_by_product_line.plot(kind='bar', color=[
                                   'skyblue', 'lightgreen', 'lightcoral', 'gold', 'lightsteelblue', 'orchid', 'grey'])
...
threshold = 5
def autopct(p): return '{:.1f}%'.format(p) if p > threshold else ''


unique_colors = ['pink', 'skyblue', 'lightgreen', 'orange', 'yellow', 'red']
wedges, texts, autotexts = plt.pie(
    status_distribution, autopct=autopct, startangle=90, colors=unique_colors, labels=None)
legend_labels = status_distribution.index + \
    ' (' + status_distribution.map(
        lambda x: f'{x / status_distribution.sum() * 100:.1f}%') + ')'
...
fig.suptitle(r'$\bf{Automobiles\ Sales\ and\ Statistics}$',
